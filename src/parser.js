...
export function isServerSideFile(filePath) {
  return filePath.includes('.server.') || 
         filePath.endsWith('.remote.ts') || 
         filePath.endsWith('.remote.js') || 
...
export function isRemoteFunctionFile(filePath) {
  return filePath.includes('.remote.') ||
         filePath.endsWith('.remote.ts') ||
         filePath.endsWith('.remote.js');
}

/**
 * Remote functions have the client part generated by sveltekit and has not to be wrapped
 * Check if the given CallExpression is a remote function wrapper like prerender(), query(), command(), form()
 */
export function isRemoteFunctionCode(init) {
  return t.isIdentifier(init.callee) && 
         ['prerender', 'query', 'command', 'form'].includes(init.callee.name);
}
...
function analyzeFunctionDeclarator(declarator, filePath, startLine, endLine) {
  ...
  if (functionType === 'remoteFunction' && t.isCallExpression(init)) {
    if (!isRemoteFunctionCode(init)) {
      return null;
    }

    let wrappedFunction = null;
    let functionArgumentIndex = 0;

    if (init.arguments.length > 1) {
      wrappedFunction = init.arguments[1]; // Second argument is the function
      functionArgumentIndex = 1;
    } else if (init.arguments.length === 1) {
      wrappedFunction = init.arguments[0]; // First argument is the function
      functionArgumentIndex = 0;
    }
    ...
    return {
      ...
      startLine,
      endLine,
      isAsync: wrappedFunction ? isAsyncFunction(wrappedFunction) : false,
      wrapperFunction: t.isIdentifier(init.callee) ? init.callee.name : null,
      functionArgumentIndex, // Store which argument contains the function
    };
  }
...
function determineFunctionType(name, filePath) {
  ...
  } else if (isRemoteFunctionFile(filePath)) {
    // Remote functions - any exported function in .remote.ts files
    // Skip special SvelteKit exports and common non-function exports
    if (!['load', 'actions', 'config', 'default'].includes(name)) {
